    ...
    public ActionResultType useOn(ItemUseContext ctx) {
        ...
        if (!world.isClientSide) {
            ...
            if (te instanceof FurnaceTileEntity || te instanceof AbstractSmeltingTileEntity) {
                ...
                if (newTe instanceof AbstractSmeltingTileEntity) {
                    ((IInventory) newTe).setItem(3, upgrade);
                    ((IInventory) newTe).setItem(4, upgrade1);
                    ((IInventory) newTe).setItem(5, upgrade2);
                    ((AbstractSmeltingTileEntity) newTe).fields.set(0, furnaceBurnTime * ((AbstractSmeltingTileEntity) newTe).getCookTime() / cooktimetotal);
                    ((AbstractSmeltingTileEntity) newTe).fields.set(1, currentItemBurnTime);
                    ((AbstractSmeltingTileEntity) newTe).fields.set(2, cooktime);
                    ((AbstractSmeltingTileEntity) newTe).fields.set(4, show);
                    for (int i = 0; i < ((AbstractSmeltingTileEntity) newTe).furnaceSettings.size(); i++) {
                        ((AbstractSmeltingTileEntity) newTe).furnaceSettings.set(i, settings[i]);
                    }
                }
