...
@OnlyIn(Dist.CLIENT)
public abstract class FuelVerifierScreen<T extends FuelVerifierContainer> extends ContainerScreen<T> {
    ...
    public FuelVerifierScreen(T t, PlayerInventory inv, ITextComponent name) {
        super(t, inv, name);
        playerInv = inv;
        this.name = name;
    }
    public static class FuelVerifierScreenDefinition extends FuelVerifierScreen<FuelVerifierContainer> {
        public FuelVerifierScreenDefinition(FuelVerifierContainer container, PlayerInventory inv, ITextComponent name) {
    ...
    protected void renderLabels(MatrixStack matrix, int mouseX, int mouseY) {
        ...
        ITextComponent burntime = new TranslationTextComponent("gui.betterfurnacesreforged.fuel.melts").append(new StringTextComponent(String.valueOf(((FuelVerifierContainer) this.getMenu()).getBurnTime()))).append( new TranslationTextComponent("gui.betterfurnacesreforged.fuel.items"));
    ...
    protected void renderBg(MatrixStack matrix, float partialTicks, int mouseX, int mouseY) {
        ...
        int i;
        i = ((FuelVerifierContainer) this.getMenu()).getBurnTimeScaled(26);
        if (i > 0) {
