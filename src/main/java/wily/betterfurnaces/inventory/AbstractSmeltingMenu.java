...
public abstract class AbstractSmeltingMenu extends AbstractInventoryMenu<AbstractSmeltingBlockEntity> {
    ...
    public int getFluidStoredScaled(int pixels, boolean isXp) {
        ...
        int cur = be.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, facing).resolve().get().getFluidInTank(0).getAmount();
        int max = be.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, facing).resolve().get().getTankCapacity(0);
        return cur * pixels / max;
    }
    public FluidStack getFluidStackStored(boolean isXp) {
        Direction facing = null;
        if (isXp) facing = DirectionUtil.fromId(be.getIndexFront());
        return be.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, facing).resolve().get().getFluidInTank(0);
    }
    public int getEnergyStoredScaled(int pixels) {
        int cur = be.getCapability(CapabilityEnergy.ENERGY, null).resolve().get().getEnergyStored();
        int max = be.getCapability(CapabilityEnergy.ENERGY, null).resolve().get().getMaxEnergyStored();
        return cur * pixels / max;
    }
    public int getEnergyStored() {
        return be.getCapability(CapabilityEnergy.ENERGY, null).resolve().get().getEnergyStored();
    }
    public int getEnergyMaxStored() {
        return be.getCapability(CapabilityEnergy.ENERGY, null).resolve().get().getMaxEnergyStored();
    }
