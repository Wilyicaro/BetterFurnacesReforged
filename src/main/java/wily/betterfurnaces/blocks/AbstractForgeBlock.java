...
public abstract class AbstractForgeBlock extends Block implements IWaterLoggable {
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult p_225533_6_) {
        ...
        if (world.isClientSide) {
            ...
            } else if ((te.hasUpgrade(Registration.LIQUID.get())) && hand.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY).isPresent() && AbstractSmeltingTileEntity.isItemFuel(hand) && !(player.isCrouching())) {
    ...
    private ActionResultType interactUpgrade(World world, BlockPos pos, PlayerEntity player, Hand handIn, ItemStack stack) {
        ...
        if (hand instanceof LiquidFuelUpgradeItem || hand instanceof EnergyFuelUpgradeItem) {
            ...
            if (te.hasUpgradeType((UpgradeItem) stack.getItem())) {
                if (!player.isCreative())
                    InventoryHelper.dropItemStack(world, pos.getX(), pos.getY() + 1, pos.getZ(), te.getUpgradeTypeSlotItem((UpgradeItem) stack.getItem()));
                else te.getUpgradeTypeSlotItem((UpgradeItem) stack.getItem()).shrink(1);
            }
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState oldState, boolean p_196243_5_) {
        if (state.getBlock() != oldState.getBlock()) {
            ...
            if (te instanceof AbstractForgeTileEntity) {
                InventoryHelper.dropContents(world, pos, (AbstractForgeTileEntity) te);
                ((AbstractForgeTileEntity) te).grantStoredRecipeExperience(world, Vector3d.atCenterOf(pos));
    ...
    private int calculateOutput(World worldIn, BlockPos pos, BlockState state) {
        AbstractForgeTileEntity tile = ((AbstractForgeTileEntity) worldIn.getBlockEntity(pos));
    ...
    public int getDirectSignal(BlockState blockState, IBlockReader world, BlockPos pos, Direction direction) {
        AbstractForgeTileEntity furnace = ((AbstractForgeTileEntity) world.getBlockEntity(pos));
        if (furnace != null) {
