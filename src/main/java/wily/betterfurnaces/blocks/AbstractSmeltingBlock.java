...
public abstract class AbstractSmeltingBlock extends Block {
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState p_180633_3_, @Nullable LivingEntity entity, ItemStack stack) {
        if (entity != null) {
            AbstractSmeltingTileEntity te = (AbstractSmeltingTileEntity) world.getBlockEntity(pos);
    ...
    protected ActionResultType interactUpgrade(World world, BlockPos pos, PlayerEntity player, Hand handIn, ItemStack stack) {
        ...
        if (!(world.getBlockEntity(pos) instanceof AbstractSmeltingTileEntity)) {
            return ActionResultType.SUCCESS;
        }
        ...
        ((AbstractSmeltingTileEntity)te).onUpdateSent();
        return ActionResultType.SUCCESS;
    }
    ...
    private int calculateOutput(World worldIn, BlockPos pos, BlockState state) {
        AbstractSmeltingTileEntity tile = ((AbstractSmeltingTileEntity)worldIn.getBlockEntity(pos));
    ...
    public int getDirectSignal(BlockState blockState, IBlockReader world, BlockPos pos, Direction direction) {
        AbstractSmeltingTileEntity furnace = ((AbstractSmeltingTileEntity) world.getBlockEntity(pos));
        if (furnace != null)
        {
