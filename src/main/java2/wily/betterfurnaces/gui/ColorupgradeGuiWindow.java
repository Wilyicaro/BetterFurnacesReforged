...
@OnlyIn(Dist.CLIENT)
public class ColorupgradeGuiWindow extends ContainerScreen<ColorupgradeGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = ColorupgradeGui.guistate;
	public ColorupgradeGuiWindow(ColorupgradeGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	protected void drawGuiContainerForegroundLayer(MatrixStack ms, int mouseX, int mouseY) {
		this.font.drawString(ms, new ItemStack(ColorupgradeItem.block).getDisplayName().getString(), 6, 4, -10066330);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 56, this.guiTop + 67, 60, 20, new StringTextComponent("Change"), e -> {
			if (true) {
				BetterfurnacesreforgedMod.PACKET_HANDLER.sendToServer(new ColorupgradeGui.ButtonPressedMessage(0, x, y, z));
				ColorupgradeGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
	}
}
