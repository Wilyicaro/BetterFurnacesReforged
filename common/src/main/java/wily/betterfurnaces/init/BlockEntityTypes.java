...
public class BlockEntityTypes {
    ...
    public static final RegistrySupplier<BlockEntityType<SmeltingBlockEntity>> BETTER_FURNACE_TILE = BLOCK_ENTITIES.register("furnace", () -> BlockEntityType.Builder.of(SmeltingBlockEntity::new, FURNACES.stream().map(Supplier::get).collect(Collectors.toList()).toArray(new Block[0])).build(null));
    public static final RegistrySupplier<BlockEntityType<ForgeBlockEntity>> FORGE_TILE = BLOCK_ENTITIES.register("forge", () -> BlockEntityType.Builder.of(ForgeBlockEntity::new, FORGES.stream().map(Supplier::get).collect(Collectors.toList()).toArray(new Block[0])).build(null));
