...
public class FactoryUpgradeWindow extends FactoryScreenWindow<SmeltingScreen<?>> {
    ...
    private FactoryDrawableButton getSideButton(int setting, int index, BlockSide side){
        return new FactoryDrawableButton(getX() + 8 + 14* (sidePositionIndex(side) % 3),getY() + 22 + 14 * ((int)Math.ceil((double)(sidePositionIndex(side) + 1 )/ 3) - 1),getSideButtonDrawable(index)).tooltips(isShiftKeyDown() ? ImmutableList.of(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_reset")) : ImmutableList.of(side.getComponent(),getTooltip(setting))).onPress((b, i)-> {
    ...
    public void renderToolTip(PoseStack poseStack, int i, int j) {
        if (parent.getMenu().getRedstoneMode() == 4)
            ScreenUtil.drawString(poseStack,parent.getMenu().getComSub() + "",getX() + (parent.getMenu().getComSub() > 9 ? 39 : 42),getY() + 86,0xFFFFFF,true);
    }
