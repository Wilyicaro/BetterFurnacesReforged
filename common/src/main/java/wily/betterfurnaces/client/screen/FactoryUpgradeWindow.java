...
public class FactoryUpgradeWindow extends FactoryScreenWindow<SmeltingScreen<?>> {
    ...
    private FactoryDrawableButton getSideButton(int setting, int index, BlockSide side){
        return new FactoryDrawableButton(x + 8 + 14* (sidePositionIndex(side) % 3),y + 22 + 14 * ((int)Math.ceil((double)(sidePositionIndex(side) + 1 )/ 3) - 1),getSideButtonDrawable(index)).tooltips(isShiftKeyDown() ? ImmutableList.of(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_reset")) : ImmutableList.of(side.getComponent(),getTooltip(setting))).onPress((b, i)-> {
