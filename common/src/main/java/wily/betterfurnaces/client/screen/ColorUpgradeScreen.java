...
public class ColorUpgradeScreen extends AbstractUpgradeScreen<ContainerColorUpgrade> {
    ...
    public List<Renderable> getNestedRenderables() {
        List<Renderable> list = new ArrayList<>(ImmutableList.of(new FactoryDrawableButton(leftPos + 8,topPos + 8, BetterFurnacesDrawables.BUTTON).icon(BetterFurnacesDrawables.getButtonIcon(12 + buttonstate)).tooltip((buttonstate == 0 ?Blocks.FURNACE : Registration.EXTREME_FORGE.get()).getName()).onPress((b, i)-> buttonstate =(buttonstate == 1 ? 0 : 1))));
    ...
    protected void renderColorFurnace(PoseStack matrix, float partialTicks, int mouseX, int mouseY) {
        ...
        stack.setTag(tag);
        matrix.pushPose();
        matrix.translate(((float) width / 2 ), (this.topPos - 40),0);
        matrix.scale(4F,4F,1F);
        matrix.translate(-8, -8,0);
        itemRenderer.renderGuiItem(matrix,stack,0, 0);
        matrix.popPose();
    }
